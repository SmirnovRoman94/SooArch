<template>
    <section>
        <div class="container">
            <div class="post_edit">
                <h1 class="form_header">Editing post!</h1>
                ...
                <form>
                    <div class="post_content">
                        ...
                        <input type="text" :class="{'error': error}" ref="elForm" v-model="post.desc" placeholder="Красткое содержание поста"/>
                        <input type="text" :class="{'error': error}" ref="elForm" v-model="post.url" placeholder="Ссылка изображения поста"/>
                        <div class="new_url">
                    ...
                    <div class="post_content">
                        ...
                        <button class="tag_item"  @click.prevent="deleteTag(item)" v-for="item in postEdit.tags" :key="item.id">{{item.name}}</button>
                    </div>
                    <div class="post_content">
                    ...
                    <div class="post_content">
                        ...
                        <div class="delete_point">
                            <div class="point_content" v-for="item in postEdit.description" :key="item.id">
                                <p>{{item.number}}</p>
                                <p>{{item.title}}</p>
                                <p>{{item.desc}}</p>
                                ...
                                <div v-if="editpoint === item">
                                    <form>
                                        <p v-show="editingPoint" class="editing">Изменения сохранены!</p>
...
export default {
    ...
    data(){
     ...
     methods: {
        ...
        deleteTag(item){
            let index = this.postEdit.tags.indexOf(item)
            if(index >-1){
                this.postEdit.tags.splice(index,1)
            }
        },
        deletePoint(item){
            let index = this.postEdit.description.indexOf(item)
            if(index > -1){
                this.postEdit.description.splice(index,1)
            }
        },
        editPoint(item){
